...
A project that ran SkillForge carries two very different things: its
**machinery** (dead weight, and actively harmful once the bundle is gone) and
its **state** (memory, stages, backlog — the valuable part). Migration sweeps
the first and preserves the second, and never confuses them.
...
2. **Project state is never touched.** Memory notes, stages, backlog, parked
   items, `_user` skills, product source — all yours, all untouched.
3. **Ambiguity is flagged, not guessed.** A script does not get to decide
   whether `prompts/` is yours. It tells you and leaves it alone.
...
```sh
cd /path/to/project
tar -xzf keel_1.5.0.tgz && bash keel/install.sh
```
...
`skills/_user/`), and reports the residue it found — **without moving
anything**.
...
**3. Sweep**, in Claude Code, by invoking the `migrate` skill (or directly):
...
The kernel changed underneath a codebase that was built under different rules.
That matters more than it sounds, because of what SkillForge recorded about
itself:
...
- Its approval gates and artifact checks **were routinely bypassed under time
  ...
  describing runs that marked units green with no machine-verifiable artifact.
- Its memory writes silently failed while the embedding daemon was degraded, so
  the record of that period is thinner than it looks.

**Work that shipped "green" under the old system may never have been verified
at all.** Do not inherit those verdicts. After the sweep, run:
...
2. **`audit`** — one scoped audit per surface that matters. Findings go to
   `BACKLOG.md`. Respect the WIP limit: at 20+ open findings, burn down before
   auditing more.
3. **`memory-consolidation`** — the memory survived a system change. Repair dead
   `[[wikilinks]]`, fold `signals/` notes into the Keel layout, and drop notes
   that only described the predecessor's machinery.

The `migrate` skill files that re-audit into `BACKLOG.md` automatically, so it
